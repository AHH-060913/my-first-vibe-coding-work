...
export const api = {
  ...
  stockDetail: (code: string, market?: string) => (STATIC_DEMO ? staticApi.stockDetail(code, market) : fetchJson<StockDetail>(`/stocks/${code}${market ? `?market=${encodeURIComponent(market)}` : ""}`)),
  resolveStock: (code: string, market?: string) => (STATIC_DEMO ? staticApi.resolveStock(code, market) : fetchJson<StockDetail>(`/stocks/${code}/resolve${market ? `?market=${encodeURIComponent(market)}` : ""}`)),
